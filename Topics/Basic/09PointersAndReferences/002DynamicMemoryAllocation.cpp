...
int main (){
  ...
  int *int_ptr {nullptr}; // this is on stack (in main() function) point to the . this pointer will hold address of integers and now its pointing to nowhere (as initialized to nullptr)
  ...
  cout << int_ptr << endl; // 0x7fe17f4057e0 -  memory location where that integer is on the heap. address that is stored in int_ptr and int_ptr is pointing to it.

  cout << *int_ptr << endl; // 41188048 - garbage - Thats because the integer that was allocated hasent been initialized. So we will assign value to that integer on the heap
  ...
  *int_ptr = 100;
  ...
  array_ptr = new double[size]; // allocate array on the heap and stores the address of first one in array_ptr
