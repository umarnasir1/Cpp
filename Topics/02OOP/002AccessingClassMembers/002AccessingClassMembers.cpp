...
public: // By default all of the information in the class is private, i.e., we dont have access to it. Now making it public, i.e., it will be accessable - if private we will get compiler error.
  ...
  //methods
  void talk(std::string text_to_say) {std::cout << name << " says " << text_to_say << std::endl;} // whatever string we pass into the player, the player will talk it right back out.
  bool is_dead();
};
...
public:
  ...
  bool deposit(double bal){balance += bal; std::cout << "Deposited: " << bal << "\nYour Balance is: " << balance <<  std::endl; } // will return true if successful or false otherwise.
  bool withdraw(double bal){balance -= bal; std::cout << "Withdrawed: " << bal << "\nYour Balance is: " << balance << std::endl; } // will return true if successful or false otherwise
};
...
int main(){
  ...
  std::cout << bjarne.name << std::endl;
  std::cout << bjarne.health << std::endl; 
  std::cout << bjarne.xp << std::endl; 
